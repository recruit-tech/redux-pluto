...
export const showOnScroll = (ComposedComponent) => class ShowOnScroll extends Component {
  ...
  scrollListener() {
    ...
    const shown = this.isShown(el.top, el.bottom, headerBottom, height, this.heightRatio);
    if (shown && this.props.onShow) {
      this.props.onShow(element);
    }

    const isInWindow = this.isInWindow(el.top, el.bottom, headerBottom, height, this.heightRatio);
  ...
  isShown(elTop, elBottom, headerBottom, windowHeight, heightRatio) {
    ...
    const isInWindow = this.isInWindow(elTop, elBottom, headerBottom, windowHeight, heightRatio);
    return isTopShown || isBottomShown || isInWindow;
  }

  isInWindow(elTop, elBottom, headerBottom, windowHeight, heightRatio) {
    const isInWindow = (elTop < headerBottom && windowHeight * heightRatio < elBottom);
    return isInWindow;
  }
...
export const forceScroll = (ComposedComponent) => class ForceScroll extends Component {
  componentDidMount() {
    ...
    // FIXME: 即座に scrollTo() を呼び出してもスクロールが機能しないため遅延させている
    typeof x === 'number' && typeof y === 'number' && setTimeout(() => window.scrollTo(x, y), 0);
  }
